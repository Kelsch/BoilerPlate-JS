(function() {
    ...
    // import { createGoogleButton } from '../../build/app.bundle.js';

    const getCalendar = async () => {
        var currentDay = new Date();
        
        // calendarFetch(currentDay);
        calendarFetch(currentDay).then(() => {
            let currentDayFirstRun = currentDay;
            currentDayFirstRun = new Date(currentDayFirstRun.setHours(0, 0, 0, 0));
            let currentDayFirstRunIdString = currentDayFirstRun.toISOString().substring(0, currentDayFirstRun.toISOString().lastIndexOf('T') + 1) + '00:00:00';
            
            checkElement(currentDayFirstRunIdString).then((element) => 
            {
    ...
    function createCalendarDay(calendarJson, selectedDate) {
        ...
        // console.log(calendar, calendarJson, startIndex, firstSunday)
        
        jobIndicators.then((jobs) => {
            dateGrid.innerHTML = '';
            for (let i = 0; i < calendar.length; i++){
                ...
                let filteredJobs = jobs.filter(job => job.installDate == obj.dateId);
                ...
                for (let index = 0; index < jobCount; index++) {
                    ...
                    // console.log(filteredJobs[index])
                    jobIndicator.setAttribute('associated-job', JSON.stringify(filteredJobs[index]));
    ...
    async function getJobList() {
        const jobIndicatorUrl = new URL(`https://pdwebapi-mf5.conveyor.cloud/api/installerAppData/getInstallIndicators?businessId=2`);
        const fetchedData = await fetch(jobIndicatorUrl);
        const jobJson = await fetchedData.json();
    ...
    function selectDay(event) {
        ...
        if (event instanceof Date) {
            ...
            if (eventDate instanceof Date && !isNaN(eventDate)) {
                ...
                let eventDateIdString = eventDate.toISOString().substring(0, eventDate.toISOString().lastIndexOf('T') + 1) + '00:00:00';
                eventDate = document.getElementById(eventDateIdString);
            }
        }
        var target = eventDate != null ? eventDate : event.target;
        ...
        ChangeContainerDate(target.getAttribute("id"));
        
        createButtons(target.querySelectorAll('.dateNumber-indicator'));
    }
    ...
    function createButtons(jobIndicatorElements) {
        let loadContainer = document.querySelector('.loadContainer');
        // console.log(jobIndicatorElements, loadContainer);
        
        // let truckName = "Small Truck Test";
        // let cabCount = 5;
        ...
        for (let i = 0; i < jobIndicatorElements.length; i++) {
            const data = JSON.parse(jobIndicatorElements[i].getAttribute('associated-job'));
            const dataString = jobIndicatorElements[i].getAttribute('associated-job');

            buttonHtml = `<button class="load mdc-button mdc-button--unelevated" associated-job='${dataString}'>
                                <div class="mdc-button__ripple"></div>
                                <span class="loadName mdc-button__label">
                                    ${data.orderId} ${data.shopName}: ${data.cabinetCount}
                                </span>
                            </button>`;
                            
            loadContainer.innerHTML += buttonHtml;
        }
    ...
    function checkKey(e) {
        if (document.getElementById('boilerPlate_modal').classList.contains('loadDetailsContainer-show')) {
            return;
        }
