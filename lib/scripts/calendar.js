(function() {
                        ...
                        for (let i = 0; i < urlArray.length; i++) {
                            ...
                            await cache.match(urlElement).then(async oldItem => {
                                await oldItem.json().then(oldItemData => {
                                    calendarJson = [...new Set([...calendarJson, ...oldItemData])];
    ...
    async function getCalendarMonth(urlMonth, monthParams) {
        ...
        // let cacheData;
        // let calendarJsonArray;
        // let monthResponse;

        // await fetch(urlMonth, {
        //     method: 'post',
        //     body: JSON.stringify(monthParams),
        //     headers: {
        //         'Content-Type': 'application/json'
        //     }
        // })
        ...
        //             if (firstFromLastAdded.dateId == result[0].dateId) {
        //                 return;
        //             }
        //         }
        ...
        //             }
        //         }
        //         firstFromLastAdded = result[0];
        //     });

        //     caches.open(cacheName).then(cache =>  {
        //         cache.put(urlMonth, newResponse);
        //     });
        // });
    ...
    function createCalendarDay(calendarJson, selectedDate) {
        ...
        for (let i = 0; i < calendar.length; i++){
            ...
            dateNumber.textContent = `${obj.dd == 1 ? month : ''} ${obj.dd} ${!touchDevice ? obj.holidayName : ''}`;
