...
// let hiddenModalSelected = Array.prototype.filter.call(hiddenModal, (filterString) => "loadDetailsContainer-show");
...
for (let index = 0; index < hiddenModals.length; index++) {
    ...
    console.log(hiddenModals, hiddenModal)
...
// let resizable = hiddenModal.querySelector('.loadDetailsContainer'),
//     resizer = hiddenModal.querySelector('.loadDetails-loadName'),
//     startY, startHeight;
...
function resetModal() {
    ...
    for (let index = 0; index < hiddenModals.length; index++) {
        const element = hiddenModals[index];
        element.classList.remove('loadDetailsContainer-show');
    }
